...
from .controls import Control, IntegerMenuItem, Item, Menu, MenuItem
from .v4l2 import *
...
class Device:
    ...
    def get_format(self) -> Tuple[ColorFormat, FrameSize]:
        ...
        with open(self.path) as f_cam:
            ...
            color_format = next(
                filter(
                    lambda x: x.pixelformat == fmt.fmt.pix.pixelformat,
    ...
    def set_format(self, color_format: ColorFormat, frame_size: FrameSize) -> None:
    ...
    def set_control_value(
        self, control: Type[Control], value: bool | int | str | Type[Item]
    ) -> None:
        """Set control value
        ...
        if any(control == available_ctrl for available_ctrl in self._controls):
            if isinstance(control, Menu):
                if isinstance(value, Item):
                    if any(item.index == value.index for item in control.items):
                ...
                if control.type in [V4L2_CTRL_TYPE_INTEGER, V4L2_CTRL_TYPE_INTEGER64]:
                    if type(value) == int:
                        if (
                            value >= control.minimum
                            and value <= control.maximum
                            and (control.minimum - value) % control.step == 0
                        ):
                            self._set_value(control, value)
                        else:
                        ...
                        raise WrongValueType(value)
                elif control.type in [V4L2_CTRL_TYPE_BOOLEAN, V4L2_CTRL_TYPE_BUTTON]:
                    if type(value) == bool:
                        self._set_value(control, value)
                    else:
                        raise WrongValueType(value)
                    ...
                    if type(value) == str:
                        if (
                            len(value) >= control.minimum
                            and len(value) <= control.maximum
                            and (control.minimum - len(value)) % control.step == 0
                        ):
                            self._set_value(control, value)
                        else:
    ...
    def get_control_value(
        ...
        if any(control == available_ctrl for available_ctrl in self._controls):
            ...
            if control.type in [V4L2_CTRL_TYPE_MENU, V4L2_CTRL_TYPE_INTEGER_MENU]:
                return next(filter(lambda x: x.index == ctrl.value, control.items))
            elif control.type in [V4L2_CTRL_TYPE_BITMASK, V4L2_CTRL_TYPE_INTEGER64]:
                return ctrl.value64
            elif control.type == V4L2_CTRL_TYPE_STRING:
    ...
    def reset_control_to_default(self, control: Type[Control]) -> None:
        ...
        if any(control == available_ctrl for available_ctrl in self._controls):
            if control.type in [
                ...
                elif control.type in [V4L2_CTRL_TYPE_MENU, V4L2_CTRL_TYPE_INTEGER_MENU]:
                    default_value = next(
                        filter(lambda x: x.index == default_value, control.items)
                    )
    ...
    def update_control(self, control: Type[Control]) -> Type[Control]:
        ...
        """
        if not any(control == available_ctrl for available_ctrl in self._controls):
            raise UnsupportedControl(self.path, control)
        ...
        with open(self.path) as f_cam:
            ...
            if ctrl.type in [V4L2_CTRL_TYPE_MENU, V4L2_CTRL_TYPE_INTEGER_MENU]:
                ...
                ctrl_idx = next(
                    iter([idx for idx, el in enumerate(self._controls) if el == ctrl]),
                    -1,
                )
    ...
    def get_available_frame_intervals(
        self, color_format: ColorFormat, frame_size: FrameSize
    ) -> List[FrameInterval]:
    ...
    def set_frame_interval(self, interval: FrameInterval) -> None:
        ...
        if not any(
            interval == available_interval for available_interval in available_intervals
        ):
    ...
    def _get_available_formats(self) -> None:
        ...
        with open(self.path) as f_cam:
            ...
            while True:
                ...
                color_format = ColorFormat(
                    fmt.description.decode(), fmt.pixelformat, fmt.flags
                )
                ...
                while True:
                    ...
                    if frmsize.type == V4L2_FRMSIZE_TYPE_DISCRETE:
                        self._available_formats[color_format].append(
                            FrameSize(frmsize.discrete.width, frmsize.discrete.height)
                        )
...
class UnsupportedColorFormat(ValueError):
    def __init__(self, device_path: str | Path, color_format: ColorFormat) -> None:
        self.device_path = device_path
...
class UnsupportedFrameSize(ValueError):
    def __init__(
        self, device_path: str | Path, color_format: ColorFormat, frame_size: FrameSize
    ) -> None:
        self.device_path = device_path
        self.color_format = color_format
...
class UnsupportedControl(ValueError):
    def __init__(self, device_path: str | Path, control: Type[Control]) -> None:
        self.device_path = device_path
...
class UnsupportedMenuItem(ValueError):
    ...
    def __str__(self) -> str:
        if isinstance(self.item, MenuItem):
            return (
                f"Menu '{self.menu.name}' does not support '{self.item.name}' menu item"
            )
        else:
...
class WrongFrameInterval(ValueError):
    def __init__(
        self, interval: FrameInterval, color_format: ColorFormat, frame_size: FrameSize
    ) -> None:
